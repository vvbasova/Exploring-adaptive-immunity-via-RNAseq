...
def fisher_by_sample(df: pd.DataFrame, target_gene: str) -> pd.DataFrame:
    ...
    sample_info = df[['tcr_sample', 'tcr_project_id']].drop_duplicates().set_index('tcr_sample')
...
def analyse_locus(
    ...
    df_locus['n_epitopes'] = (
        df_locus
        .groupby(['tcr_sample', 'tcr_cdr3'])['vdj_antigen.epitope']
        .transform('nunique')
        .fillna(1)
    )
    df_locus['weighted_count'] = df_locus['tcr_duplicate_count'] / df_locus['n_epitopes']
    ...
    epi_stats = df_locus.groupby('vdj_antigen.epitope').agg(
        ...
        antigen_species=('vdj_antigen.species', lambda x: Counter(x).most_common(1)[0][0]),
    ...
    epi2clones = df_locus.groupby('vdj_antigen.epitope')['tcr_cdr3'].apply(set).loc[epi_stats.index]
    ...
    for epitope, epi_set in epi2clones.items():
        ...
        for proj, pstats in proj_stats.iterrows():
            ...
            rows.append((
                ...
                pstats['dup_norm'], dup_epi,
                k, overlap_dup, x_val, y_val, p_raw
            ))

    res = pd.DataFrame(rows, columns=[
        'project_id', 'epitope', 'antigen_species', 'antigen_gene',
        'dup_project_norm', 'dup_epitope',
        'overlap_unique', 'overlap_dup', 'x', 'y', 'p_raw'
    ])
    ...
    if plot:
        ...
        sns.scatterplot(
            data=res, x='x', y='y', hue='project_id',
            palette='tab20', s=60, alpha=0.75, edgecolor='none', legend=False
        )
